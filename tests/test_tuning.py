...
# `tiny_settings` and `tiny_batches` (and the `_synthetic_panel` helper they share with
# `healthy_batches` below) now live in `tests/conftest.py` -- `test_sentences.py` needs the
# exact same tiny synthetic panel, and two copies of it would only ever drift apart, never
# stay identical on purpose.
from conftest import _synthetic_panel
